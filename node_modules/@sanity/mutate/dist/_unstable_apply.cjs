...
var arrify = require("./_chunks-cjs/arrify.cjs"), utils = require("./_chunks-cjs/utils.cjs");
...
const empty = {}, createStore = (initialEntries) => {
  let version = 0, index = initialEntries && initialEntries?.length > 0 ? Object.fromEntries(
    initialEntries.map((entry) => {
      const doc = update(utils.assignId(entry, utils.nanoid), utils.nanoid());
      return [doc._id, doc];
    })
  ) : empty;
  return {
...
function createIn(index, mutation) {
  const document = utils.assignId(mutation.document, utils.nanoid);
