"use strict";
var stringify = require("./stringify.cjs"), isObject = require("./isObject.cjs"), diffMatchPatch$1 = require("@sanity/diff-match-patch"), node_crypto = require("node:crypto");
...
function fillPool(bytes) {
  !pool || pool.length < bytes ? (pool = Buffer.allocUnsafe(bytes * POOL_SIZE_MULTIPLIER), node_crypto.webcrypto.getRandomValues(pool), poolOffset = 0) : poolOffset + bytes > pool.length && (node_crypto.webcrypto.getRandomValues(pool), poolOffset = 0), poolOffset += bytes;
}
