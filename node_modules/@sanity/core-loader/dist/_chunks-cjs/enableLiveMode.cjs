"use strict";
var client = require("@sanity/client"), stega = require("@sanity/client/stega"), comlink = require("@sanity/comlink"), presentationComlink = require("@sanity/presentation-comlink"), index = require("./index.cjs");
const LISTEN_HEARTBEAT_INTERVAL = 1e4;
function enableLiveMode(options) {
  const { client: client$1, setFetcher, onConnect, onDisconnect, onPerspective } = options;
  if (!client$1)
    throw new Error(
      `Expected \`client\` to be an instance of SanityClient: ${JSON.stringify(client$1)}`
    );
  const { projectId, dataset, perspective } = client$1.config(), $perspective = index.atom(
    perspective && perspective !== "raw" ? perspective : "drafts"
  ), $connected = index.atom(!1), cache = /* @__PURE__ */ new Map(), comlink$1 = comlink.createNode(
    {
      name: "loaders",
  ...
  comlink$1.onStatus(() => {
    $connected.set(!0);
  }, "connected"), comlink$1.on("loader/perspective", (data) => {
    if (data.projectId === projectId && data.dataset === dataset) {
      client.validateApiPerspective(data.perspective);
  ...
  }), comlink$1.on("loader/query-change", (data) => {
    if (data.projectId === projectId && data.dataset === dataset) {
      const { perspective: perspective2, query, params } = data;
      data.result !== void 0 && data.resultSourceMap !== void 0 && client$1.config().stega.enabled ? cache.set(JSON.stringify({ perspective: perspective2, query, params }), {
        ...data,
        result: stega.stegaEncodeSourceMap(
          data.result,
          data.resultSourceMap,
          client$1.config().stega
        )
  ...
  const unlistenConnection = $connected.listen((connected) => {
    ...
    for (const { query, params, $fetch } of liveQueries)
      comlink$1.post("loader/query-listen", {
        projectId,
        dataset,
        perspective: perspective2,
        query,
  ...
  function updateLiveQueries() {
    ...
    comlink$1.post("loader/documents", {
      projectId,
      dataset,
      perspective: perspective2,
...
exports.enableLiveMode = enableLiveMode;
//# sourceMappingURL=enableLiveMode.cjs.map
